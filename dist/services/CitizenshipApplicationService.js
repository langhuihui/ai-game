...
export class CitizenshipApplicationService {
    ...
    createApplication(data) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, 'pending')
    `);
        const result = stmt.run(data.character_id, data.character_name, data.description, data.personality, data.message, data.preferred_character_id || null);
    ...
    // 根据角色ID获取申请
    getApplicationByCharacterId(characterId) {
    ...
    // 为申请生成唯一的角色名称（排除当前申请）
    generateUniqueCharacterNameForApplication(preferredName, applicationId) {
    ...
    // 检查角色ID是否唯一
    isCharacterIdUnique(characterId) {
        // 检查申请表中的ID
    ...
    // 生成唯一的角色名称，尽量使用期望的名称
    generateUniqueCharacterName(preferredName) {
        if (!preferredName) {
    ...
    // 生成多个唯一的角色名称（用于测试）
    generateMultipleUniqueCharacterNames(baseName, count) {
        const names = [];
    ...
    isCharacterNameUnique(name) {
        ...
        const result = stmt.get(name, 'approved');
        return result.count === 0;
    }
    // 检查角色名称是否唯一（不包括当前申请）
    isCharacterNameUniqueExcludingApplication(name, excludeApplicationId) {
        // 检查角色表中的名称
        const existingCharacter = this.characterService.getCharacterByName(name);
        if (existingCharacter)
            return false;
        ...
        if (excludeApplicationId) {
            ...
            const result = stmt.get(name, 'approved', excludeApplicationId);
            return result.count === 0;
        }
        else {
...
//# sourceMappingURL=CitizenshipApplicationService.js.map