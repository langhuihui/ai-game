...
import { IdentityRole, hasCapability, hasMinimumRole } from '../models/Identity.js';
...
export class IdentityService {
    ...
    createIdentity(data) {
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
        const result = stmt.run(data.character_id ?? null, data.identity_role, data.secret_key, data.expires_at ?? null, 1);
        return this.getIdentityById(result.lastInsertRowid);
    }
    // 根据ID获取身份
    getIdentityById(id) {
    ...
    // 根据秘钥获取身份
    getIdentityBySecretKey(secretKey) {
        const stmt = this.db.prepare(`
    ...
    // 根据角色ID获取身份
    getIdentityByCharacterId(characterId) {
        const stmt = this.db.prepare(`
      SELECT * FROM identities 
    ...
    // 验证身份能力
    validateCapability(secretKey, requiredCapability) {
        // 首先检查是否为固定超级管理员秘钥
    ...
    // 验证最低身份等级
    validateMinimumRole(secretKey, requiredRole) {
        // 首先检查是否为固定超级管理员秘钥
        if (this.isFixedSuperAdminKey(secretKey)) {
    ...
    // 为角色创建默认身份
    createDefaultIdentity(characterId, role = IdentityRole.VISITOR) {
