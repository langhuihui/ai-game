    ...
    static async execute(args) {
        try {
            ...
            if (behaviorManager.has('item', behaviorName)) {
                ...
                if (behavior) {
                    effect = await behavior.execute(null, character);
                }
            }
            ...
            if (effect) {
                if (effect.health_change) {
                    updatedCharacter = entityManager.getCharacterService().updateCharacterHealth(args.character_id, effect.health_change);
                    effectMessage += ` Health ${effect.health_change > 0 ? '+' : ''}${effect.health_change}.`;
                }
                if (effect.mental_state_change) {
                    updatedCharacter = entityManager.getCharacterService().updateCharacterMentalState(args.character_id, effect.mental_state_change);
            ...
            memoryService.addActionMemory(args.character_id, 'used an item', `Used: ${useResult.item.name}. ${effect?.description || 'No special effect.'}`);
            return {
                success: true,
                item: useResult.item,
                effect: effect,
