    ...
    static async execute(args) {
        try {
            ...
            if (character.current_scene_id) {
                ...
                const isConnected = connections.some((conn) => conn.to_scene_id === args.target_scene_id);
                if (!isConnected) {
                    return {
                        success: false,
            ...
            memoryService.addActionMemory(args.character_id, 'moved to new location', `Moved to: ${targetScene.name}`);
            return {
                success: true,
                character: result,
...
//# sourceMappingURL=MoveCharacterCommand.js.map