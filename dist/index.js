...
class GameServer {
    ...
    permissionConnections = new Map(); // connectionId -> secretKey
    clientConnections = new Map(); // response -> connectionId
    toolRouter;
    constructor() {
    ...
    setupPageRoutes() {
        ...
        this.webApp.get('/logs', (req, res) => {
            try {
                ...
                let logs;
                if (date) {
                    logs = this.loggingService.getLogsByDate(date);
                }
                else {
                ...
                if (character) {
                    logs = logs.filter((log) => log.character_name === character);
                }
                // Get unique character names for filter dropdown
                const characters = [...new Set(logs.map((log) => log.character_name).filter(Boolean))];
                res.render('logs', {
                    page: 'logs',
                    logs,
                    characters,
    ...
    setupWebRoutes() {
        ...
        this.webApp.get('/api/characters', (req, res) => {
            ApiResponseHandler.handleRequestWithKey(res, () => Promise.resolve(this.characterService.getAllCharacters()), 'characters');
        });
        this.webApp.get('/api/characters/:id', (req, res) => {
            ApiResponseHandler.handleRequest(res, () => {
        ...
        this.webApp.get('/api/characters/:id/memories', (req, res) => {
            ApiResponseHandler.handleRequestWithKey(res, () => Promise.resolve(this.memoryService.getAllMemories(parseInt(req.params.id))), 'memories');
        });
        this.webApp.get('/api/characters/:id/items', (req, res) => {
            ApiResponseHandler.handleRequestWithKey(res, () => Promise.resolve(this.itemService.getItemsByCharacter(parseInt(req.params.id))), 'items');
        });
        // Scenes
        this.webApp.get('/api/scenes', (req, res) => {
            ApiResponseHandler.handleRequestWithKey(res, () => Promise.resolve(this.sceneService.getAllScenes()), 'scenes');
        });
    ...
    async handleMCPRequest(request, headers) {
