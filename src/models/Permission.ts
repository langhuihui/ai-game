export enum PermissionLevel {
  ...
  SUPER_ADMIN = 'super_admin' // 超级管理员 - 隐藏身份，开发者权限
}
...
// 权限验证函数
export function hasPermission(permissionLevel: PermissionLevel, requiredPermission: string): boolean {
...
// 权限等级比较
export function comparePermissionLevels(level1: PermissionLevel, level2: PermissionLevel): number {
  const levels = [
    PermissionLevel.PRISONER,
    PermissionLevel.VISITOR,
    PermissionLevel.CITIZEN,
...
// 检查是否有足够权限
export function hasMinimumPermission(userLevel: PermissionLevel, requiredLevel: PermissionLevel): boolean {
