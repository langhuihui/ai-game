      ...
      case 'move_character':
        try {
          ...
          this.memoryService.addActionMemory(
            ...
            `Moved from scene ${character.current_scene_id} to scene ${args.target_scene_id}`
          );
        ...
        try {
          ...
          if (result.effect) {
            if (result.effect.health_change) {
              updatedCharacter = this.characterService.updateCharacterHealth(
                args.character_id,
                result.effect.health_change
              )!;
              effectMessage += ` Health ${result.effect.health_change > 0 ? '+' : ''}${result.effect.health_change}.`;
            }

            if (result.effect.mental_state_change) {
              updatedCharacter = this.characterService.updateCharacterMentalState(
                args.character_id,
                result.effect.mental_state_change
              )!;
          ...
          this.memoryService.addActionMemory(
            args.character_id,
            'used an item',
            `Used: ${result.item.name}. ${result.effect?.description || 'No special effect.'}`
          );
